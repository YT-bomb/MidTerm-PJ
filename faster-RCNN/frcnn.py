...
class FRCNN(object):
    ...
    def proposal_box(self, image, crop = False, count = False):
        #---------------------------------------------------#
        #   计算输入图片的高和宽
        #---------------------------------------------------#
        image_shape = np.array(np.shape(image)[0:2])
        ...
        with torch.no_grad():
            ...
            #   对建议框进行解码，
            #-------------------------------------------------------------#
            rois[..., [0, 2]] = (rois[..., [0, 2]]) / input_shape[1] * image_shape[0]
            rois[..., [1, 3]] = (rois[..., [1, 3]]) / input_shape[0] * image_shape[1]     
        ...
        for i in range(len(boxes)):
            ...
            for i in range(thickness):
                ...
                draw.rectangle([left + i, top + i, right - i, bottom - i], outline=self.colors[index])
            del draw
