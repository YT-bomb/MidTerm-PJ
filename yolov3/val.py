...
def run(data,
    ...
    con_matrix = confusion_matrix.matrix
    iou = np.diag(con_matrix) / (con_matrix.sum(axis=1) + con_matrix.sum(axis=0) - np.diag(con_matrix))
    miou = np.nanmean(iou)
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (mp, mr, map50, map, miou, acc,*(loss.cpu() / len(dataloader)).tolist()), maps, t
